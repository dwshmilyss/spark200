...
object SparkSubmit {
  ...
  private def runMain(
    ...
    //加载clildClassPath (--jars)
    for (jar <- childClasspath) {
      addJarToClasspath(jar, loader)
    }

    //将sysProps(合并--properties-file(没有配置默认为conf/spark-defaults.conf))设置到当前进程的环境变量里
    ...
    /**
      * 利用反射执行 childMainClass(childArgs)
      * childMainClass 即spark-submit中 --class指定的类
      */
    try {
    ...
    /**
      * 执行的类不要extends scala.App，而是写main函数
      */
    // SPARK-4170
