...
class StreamingContext private[streaming] (
  ...
  def myTextFileStream(directory: String,rememberDuration: Duration,newFilesOnly:Boolean): DStream[String] = withNamedScope("my define text file stream") {
    myFileStream[LongWritable, Text, TextInputFormat](directory,rememberDuration,newFilesOnly).map(_._2.toString)
  }
